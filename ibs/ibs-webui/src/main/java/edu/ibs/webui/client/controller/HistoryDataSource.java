...
/**
 * User: Максим Date: 11.01.13 Time: 6:18
 */
public class HistoryDataSource extends GwtRpcDataSource {
	...
	public void executeFetch(final String requestId, final DSRequest request, final DSResponse response) {
		service.getHistory(ApplicationManager.getInstance().getAccount().getUser(), card, from, to, new AppCallback<List<TransactionDTO>>() {
			...
			public void onSuccess(List<TransactionDTO> list) {
				...
				if (list != null && list.size() > 0) {
					...
					for (int i = 0; i < size; i++) {
						...
						record.setAttribute("from", dto.getFrom().toString());
						record.setAttribute("to", dto.getTo().toString());
